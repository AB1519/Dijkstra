...
public class DijkstraFind {
    ...
    private WeighedAdjacencyList graph; // graph object

    /**
     * Instantiate algorithm providing graph
     * @param graph WeighedDigraph graph
     */
    public DijkstraFind(WeighedAdjacencyList graph) {
        this.graph = graph;
        size = graph.size();
    }
    ...
    public ArrayList<Integer> shortestPath(int vertexA, int vertexB) {
        ...
        while (pq.size() > 0) {
            ...
            ArrayList<WeighedgraphEdge> neighbors = graph.edgesOf(currentNode);

            if (neighbors == null) continue;

            for (WeighedgraphEdge neighbor : neighbors) {
   ...
   // public static void main(String args[]) {
     //   WeighedAdjacencyList graph;

       // try {
         //   graph = new WeighedAdjacencyList(args[0]);
            // Print graph
